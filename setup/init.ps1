...
function RenameFiles($files) {
    foreach ($file in $files) {
        if ($file -and $file.Name.Contains($sk_name) -and (-not ($file.FullName.Contains($binDir) -or $file.FullName.Contains($objDir)))) {         
            Status($file.FullName)
            Rename-Item -Path $file.FullName  -NewName $file.Name.Replace($sk_name, $config.projectName)
        }
    }
}
    ...
    foreach ($dir in $dirs) {
        if ($dir -and $dir.Name.Contains($sk_name) -and (-not ($dir.FullName.Contains($binDir) -or $dir.FullName.Contains($objDir)))) {
            ...
            if ($relativeParentPath.Contains($sk_name)) {
                $path = Join-Path -Path $root -ChildPath $relativeParentPath.Replace($sk_name, $config.projectName)
                $path = Join-Path -Path $path -ChildPath $dir.Name
            }
        
            $replaced = $dir.Name.Replace($sk_name, $config.projectName)
            Status($path)
...
Replace $unicornFiles $sk_name $config.projectName
Info("Setup src files...")
Replace $srcFiles $sk_name $config.projectName
Info("Setup script files...")
Replace $buildFiles $sk_name $config.projectName
Info("Setup Sitecore version...")
Replace $packageFiles $sk_sitecoreVersion $config.sitecoreVersion
Info("Setup Glass Mapper version...")
Replace $packageFiles  $sk_glassMapperVersion $config.glassMapperVersion
Info("Setup .NET version...")
Replace $packageFiles  $sk_dotnetVersion $config.dotnetVersion
